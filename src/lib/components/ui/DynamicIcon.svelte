<script lang="ts">
	...
	import FlowbiteTailwindSolid from '~icons/flowbite/tailwind-solid';

	let { icon } = $props<{ icon: string }>();
</script>

<!-- Few enough icons are used in this app that we'll just pick between them -->

{#if icon === '%svelte%'}
	<CibSvelte style="color: #ff5722" />
{:else if icon === '%typescript%'}
	<CibTypescript style="color: #0088d1" />
{:else if icon === '%tailwindcss%'}
	<FlowbiteTailwindSolid style="color: #06b6d4" />
{:else if icon === '%skeleton%'}
	<!-- Skeleton icon (from their website, belongs entirely to the Skeleton team at skeleton.dev) -->
	<svg
		class="fill-token"
		width="24px"
		height="24px"
